...
import type { ChatApiPayload, ChatApiRequest, NormalizedChatResponse } from "@/types/chat";

const REQUEST_TIMEOUT_MS = 45000;
const CANDIDATE_TEXT_KEYS = ["output", "response", "answer", "message", "text", "content", "reply"];
...
export async function sendChatMessage(
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`El endpoint respondió con estado ${response.status}.`);
    }
    ...
    if (!text) {
      throw new Error(
        "No se pudo interpretar la respuesta del endpoint. Revisá DOC_DEMO.md para ajustar la normalización."
      );
    }
    ...
    if (error instanceof DOMException && error.name === "AbortError") {
      ...
      throw new Error("El endpoint tardó demasiado en responder. Intentá nuevamente.");
    }
    ...
    throw new Error("Ocurrió un error inesperado al consultar el endpoint.");
  } finally {
