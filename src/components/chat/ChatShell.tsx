...
export function ChatShell() {
  ...
  const scrollStateRef = useRef<{ assistantCount: number; wasSending: boolean } | null>(null);
  ...
  useEffect(() => {
    ...
    if (!heroQuestion || consumedQueryRef.current === heroQuestion || isSending) {
      return;
    }
  ...
  return (
    <div className="relative isolate min-h-screen overflow-hidden bg-[#0a0b0e] text-ink">
      <header className="fixed inset-x-0 top-0 z-40 border-b border-stroke-soft/80 bg-bg/80 backdrop-blur-xl">
        <div className="mx-auto flex h-20 max-w-4xl items-center justify-center px-4 sm:px-6 lg:px-8">
          <BrandLogo href="/" iconOnly onClick={handleBackToHome} className="justify-center" />
        </div>
      </header>

      <main className="relative mx-auto w-full max-w-4xl px-4 pb-28 pt-24 sm:px-6 lg:px-8">
      ...
      <div className="fixed inset-x-0 bottom-0 z-40 bg-bg/85 py-4 backdrop-blur-md">
        <div className="mx-auto w-full max-w-4xl px-4 sm:px-6 lg:px-8">
          <ChatComposer value={draft} disabled={isSending} onChange={setDraft} onSubmit={handleSubmit} />
        </div>
      </div>
    </div>
  );
